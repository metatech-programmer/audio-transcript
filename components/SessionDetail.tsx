        ...
        <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
          ...
          <div className="flex items-start justify-between gap-4 mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-extrabold text-slate-900 truncate-ellipsis max-w-dvw-80 flex-truncate">
                  {session.title}
                </h1>
            ...
            <div className="flex items-center gap-3">
              <div className="flex gap-2 items-center">
                {session.tags.map((tag) => (
                  <div
                    key={tag}
                    className="flex items-center gap-2 bg-slate-50 rounded-full px-3 py-1 text-sm border border-slate-100"
                  >
              ...
              {isEditingTags ? (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="px-3 py-1 border rounded-md bg-slate-50"
                  />
                  <button
          ...
          {summaryData && (
            <div className="space-y-10">
              ...
              <section className="rounded-2xl border bg-white p-4 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-lg font-semibold flex items-center gap-2">
                    <BookOpen size={18} /> Identificación y Metadatos
                  </h3>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm text-slate-700">
                  <div>
                    <strong>Asignatura:</strong>{" "}
                    <span className="truncate-ellipsis max-w-dvw-50 inline-block">
                      {String(
                        (summaryData.metadata && (summaryData.metadata as any).subject) || "—"
                      )}
                    </span>
                  </div>
                  <div>
                    <strong>Unidad:</strong>{" "}
                    <span className="truncate-ellipsis max-w-dvw-40 inline-block">
                      {String((summaryData.metadata && (summaryData.metadata as any).unit) || "—")}
                    </span>
                  </div>
                  <div>
                    <strong>Sesión:</strong>{" "}
                    {String(
                      (summaryData.metadata && (summaryData.metadata as any).sessionNumber) || "—"
                    )}
                  </div>
                  <div className="sm:col-span-3">
                    <strong>Eje temático:</strong>{" "}
                    <span className="truncate-ellipsis max-w-dvw-70 inline-block">
                      {String(
                        (summaryData.metadata && (summaryData.metadata as any).thematicAxis) || "—"
                      )}
                    </span>
                  </div>
                  <div className="sm:col-span-3">
                    ...
                    <span className="truncate-ellipsis max-w-dvw-70 inline-block">
                      {Array.isArray(summaryData.metadata && (summaryData.metadata as any).keywords)
                        ? (summaryData.metadata as any).keywords.join(", ")
                        : String(
                            (summaryData.metadata && (summaryData.metadata as any).keywords) || "—"
                          )}
                    </span>
                  </div>
                </div>
              ...
              <section className="rounded-2xl border bg-white p-4 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-lg font-semibold flex items-center gap-2">
                    <Check size={18} /> Objetivos de Aprendizaje
                  </h3>
                </div>
                <div className="text-sm text-slate-700">
                  <div className="mb-2">
                    ...
                    {Array.isArray(
                      summaryData.learningOutcomes &&
                        (summaryData.learningOutcomes as any).competencies
                    )
                      ? (summaryData.learningOutcomes as any).competencies.join("; ")
                      : String(
                          (summaryData.learningOutcomes &&
                            (summaryData.learningOutcomes as any).competencies) ||
                            "—"
                  ...
                  <div>
                    ...
                    {String(
                      (summaryData.learningOutcomes &&
                        (summaryData.learningOutcomes as any).mainProblem) ||
                        "—"
                    )}
                  </div>
                </div>
              ...
              <section className="rounded-2xl border bg-white p-4 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-lg font-semibold flex items-center gap-2">
                    <BookOpen size={18} /> Núcleo Teórico y Estado del Arte
                  </h3>
                </div>
                <div className="text-sm text-slate-700">
                  <div className="mb-2">
                    ...
                    {String(
                      (summaryData.theoreticalCore &&
                        (summaryData.theoreticalCore as any).concepts) ||
                        "—"
                    )}
                  </div>
                  <div className="mb-2">
                    <strong>Modelos/Estándares:</strong>{" "}
                    {String(
                      (summaryData.theoreticalCore &&
                        (summaryData.theoreticalCore as any).modelsStandards) ||
                        "—"
                    )}
                  </div>
                  <div>
                    <strong>Autores referenciados:</strong>{" "}
                    {Array.isArray(
                      summaryData.theoreticalCore &&
                        (summaryData.theoreticalCore as any).referencedAuthors
                    )
                      ? (summaryData.theoreticalCore as any).referencedAuthors.join(", ")
                      : String(
                          (summaryData.theoreticalCore &&
                            (summaryData.theoreticalCore as any).referencedAuthors) ||
                            "—"
              ...
              <section className="rounded-2xl border bg-white p-4 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-lg font-semibold flex items-center gap-2">
                    <Lightbulb size={18} /> Análisis Comparativo y Crítico
                  </h3>
                </div>
                <div className="text-sm text-slate-700">
                  <div className="mb-2">
                    ...
                    {String(
                      (summaryData.comparativeAnalysis &&
                        (summaryData.comparativeAnalysis as any).prosCons) ||
                        "—"
                    )}
                  </div>
                  <div className="mb-2">
                    ...
                    {String(
                      (summaryData.comparativeAnalysis &&
                        (summaryData.comparativeAnalysis as any).comparisons) ||
                        "—"
                    )}
                  </div>
                  <div>
                    ...
                    {String(
                      (summaryData.comparativeAnalysis &&
                        (summaryData.comparativeAnalysis as any).contextOfUse) ||
                        "—"
                    )}
                  </div>
                </div>
              ...
              <section className="rounded-2xl border bg-white p-4 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-lg font-semibold flex items-center gap-2">
                    <FileText size={18} /> Bloque de Ejemplos y Casos de Uso
                  </h3>
                </div>
                <div className="text-sm text-slate-700">
                  <div className="mb-2">
                    ...
                    {String(
                      (summaryData.examples && (summaryData.examples as any).academicExample) || "—"
                    )}
                  </div>
                  <div className="mb-2">
                    <strong>Caso de Estudio:</strong>{" "}
                    {String(
                      (summaryData.examples && (summaryData.examples as any).caseStudy) || "—"
                    )}
                  </div>
                  <div>
                    <strong>Anti-patrones:</strong>{" "}
                    {String(
                      (summaryData.examples && (summaryData.examples as any).antipatterns) || "—"
                    )}
                  </div>
                </div>
              </section>
              ...
              <section className="rounded-2xl border bg-white p-4 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-lg font-semibold flex items-center gap-2">
                    <BookOpen size={18} /> Componente Técnico / Práctico
                  </h3>
                </div>
                <div className="text-sm text-slate-700">
                  <div className="mb-2">
                    <strong>Snippets / Código:</strong>
                    <pre className="bg-slate-50 p-3 rounded mt-2 text-xs whitespace-pre-wrap">
                      {String(
                        (summaryData.technicalComponent &&
                          (summaryData.technicalComponent as any).codeSnippets) ||
                          "—"
                      )}
                    </pre>
                  </div>
                  <div className="mb-2">
                    ...
                    {Array.isArray(
                      summaryData.technicalComponent &&
                        (summaryData.technicalComponent as any).tools
                    )
                      ? (summaryData.technicalComponent as any).tools.join(", ")
                      : String(
                          (summaryData.technicalComponent &&
                            (summaryData.technicalComponent as any).tools) ||
                            "—"
                  ...
                  <div>
                    ...
                    {String(
                      (summaryData.technicalComponent &&
                        (summaryData.technicalComponent as any).diagrams) ||
                        "—"
                    )}
                  </div>
                </div>
              ...
              <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="rounded-2xl border bg-white p-4 shadow-sm">
                  ...
                  <div className="text-sm text-slate-700">
                    <div className="mb-2">
                      <strong>Puntos polémicos:</strong>{" "}
                      {String(
                        (summaryData.debate && (summaryData.debate as any).controversies) || "—"
                      )}
                    </div>
                    <div>
                      <strong>Aportes relevantes:</strong>{" "}
                      {String(
                        (summaryData.debate &&
                          (summaryData.debate as any).participantContributions) ||
                          "—"
                      )}
                    </div>
                  </div>
                ...
                <div className="rounded-2xl border bg-white p-4 shadow-sm">
                  ...
                  <div className="text-sm text-slate-700">
                    <div className="mb-2">
                      ...
                      {String(
                        (summaryData.interdisciplinary &&
                          (summaryData.interdisciplinary as any).relations) ||
                          "—"
                      )}
                    </div>
                    <div>
                      <strong>Aplicación en el trabajo:</strong>{" "}
                      {String(
                        (summaryData.interdisciplinary &&
                          (summaryData.interdisciplinary as any).workplaceApplications) ||
                          "—"
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-amber-800/70">
                        Captura concisa y accionable de la clase
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={async () => {
                        try {
                          const parsed = parseJsonFromString(
                            String(summaryData.executiveSummary || "")
                          );
                          const toCopy = parsed
                  ...
                  {(() => {
                    ...
                    if (typeof summaryData === "object" && summaryData !== null) {
                      ...
                      if (typeof execSummary === "object" && execSummary !== null) {
                        ...
                        execSummary =
                          inner.executiveSummary || inner.resumen || inner.summary || "";
                      }
                    }
                    ...
                    return sanitizeSummaryText(execSummary)
                      .split(/\n\n+/)
                      .filter(Boolean)
                      .map((p: string, idx: number) => (
                        <p key={idx} className={idx === 0 ? "text-lg font-medium" : ""}>
                          {p}
                        </p>
                      ));
                  })()}
              ...
              <section>
                ...
                {showRawJson && (
                  <pre className="bg-slate-900 text-white p-4 rounded-md text-xs overflow-auto max-h-56">
                    {JSON.stringify(session.summary, null, 2)}
                  </pre>
                )}
              </section>
            ...
            <div className="bg-white rounded-2xl border p-4">
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-bold flex items-center gap-2">
                  <FileText size={18} /> Transcripción Completa
                </h2>
                <ChevronDown size={18} />
              </div>
              {expandedSections.transcript && (
                <>
                  ...
                  <div className="mt-4 flex justify-end">
                    ...
                    >
                      {isGeneratingSummary || summarizing
                        ? "Generando resumen..."
                        : "Volver a generar resumen"}
                    </button>
                  </div>
                </>
              )}
              ...
              {expandedSections.export && (
                <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <button
                    ...
                    className="px-4 py-3 bg-slate-700 text-white rounded-md"
                  >
                    JSON
                  </button>
                  <button
...
function normalizeSummaryForRender(summary: unknown) {
  ...
  let executiveSummary = toText(
    extractedSrc.executiveSummary || extractedSrc.resumen || extractedSrc.summary || ""
  );
  let keyPoints = toList(
    extractedSrc.keyPoints || extractedSrc.puntosClave || extractedSrc.puntos_clave || []
  );
  let lectureNotes = toText(
    extractedSrc.lectureNotes || extractedSrc.notas || extractedSrc.notasDeClase || ""
  );
  let actionableInsights = toList(
    extractedSrc.actionableInsights ||
      extractedSrc.conceptosAplicables ||
      extractedSrc.insights ||
      []
  );
  ...
  if (
    ...
    if (typeof summary === "string")
      return { executiveSummary: summary, keyPoints: [], lectureNotes: "", actionableInsights: [] };
  }

  // extract high-complexity fields when present
  const metadata =
    extractedSrc.metadata && typeof extractedSrc.metadata === "object"
      ? (extractedSrc.metadata as Record<string, unknown>)
      : {
  ...
  const learningOutcomes = {
    competencies: toList(
      (extractedSrc.learningOutcomes && (extractedSrc.learningOutcomes as any).competencies) ||
        (extractedSrc as any).competencies ||
        []
    ),
    mainProblem: toText(
      (extractedSrc.learningOutcomes && (extractedSrc.learningOutcomes as any).mainProblem) ||
        (extractedSrc as any).mainProblem ||
        ""
    ),
  };
  ...
  const theoreticalCore = {
    concepts: toText(
      (extractedSrc.theoreticalCore && (extractedSrc.theoreticalCore as any).concepts) ||
        (extractedSrc as any).concepts ||
        ""
    ),
    modelsStandards: toText(
      (extractedSrc.theoreticalCore && (extractedSrc.theoreticalCore as any).modelsStandards) ||
        (extractedSrc as any).modelsStandards ||
        ""
    ),
    referencedAuthors: toList(
      (extractedSrc.theoreticalCore && (extractedSrc.theoreticalCore as any).referencedAuthors) ||
        (extractedSrc as any).referencedAuthors ||
        []
    ),
  };
  ...
  const comparativeAnalysis = {
    prosCons: toText(
      (extractedSrc.comparativeAnalysis && (extractedSrc.comparativeAnalysis as any).prosCons) ||
        (extractedSrc as any).prosCons ||
        ""
    ),
    comparisons: toText(
      (extractedSrc.comparativeAnalysis && (extractedSrc.comparativeAnalysis as any).comparisons) ||
        (extractedSrc as any).comparisons ||
        ""
    ),
    contextOfUse: toText(
      (extractedSrc.comparativeAnalysis &&
        (extractedSrc.comparativeAnalysis as any).contextOfUse) ||
        (extractedSrc as any).contextOfUse ||
        ""
    ),
  };
  ...
  const examples = {
    academicExample: toText(
      (extractedSrc.examples && (extractedSrc.examples as any).academicExample) ||
        (extractedSrc as any).academicExample ||
        ""
    ),
    caseStudy: toText(
      (extractedSrc.examples && (extractedSrc.examples as any).caseStudy) ||
        (extractedSrc as any).caseStudy ||
        ""
    ),
    antipatterns: toText(
      (extractedSrc.examples && (extractedSrc.examples as any).antipatterns) ||
        (extractedSrc as any).antipatterns ||
        ""
    ),
  };
  ...
  const technicalComponent = {
    codeSnippets: toText(
      (extractedSrc.technicalComponent && (extractedSrc.technicalComponent as any).codeSnippets) ||
        (extractedSrc as any).codeSnippets ||
        ""
    ),
    tools: toList(
      (extractedSrc.technicalComponent && (extractedSrc.technicalComponent as any).tools) ||
        (extractedSrc as any).tools ||
        []
    ),
    diagrams: toText(
      (extractedSrc.technicalComponent && (extractedSrc.technicalComponent as any).diagrams) ||
        (extractedSrc as any).diagrams ||
        ""
    ),
  };
  ...
  const debate = {
    controversies: toText(
      (extractedSrc.debate && (extractedSrc.debate as any).controversies) ||
        (extractedSrc as any).controversies ||
        ""
    ),
    participantContributions: toText(
      (extractedSrc.debate && (extractedSrc.debate as any).participantContributions) ||
        (extractedSrc as any).participantContributions ||
        ""
    ),
  };
  ...
  const interdisciplinary = {
    relations: toText(
      (extractedSrc.interdisciplinary && (extractedSrc.interdisciplinary as any).relations) ||
        (extractedSrc as any).relations ||
        ""
    ),
    workplaceApplications: toText(
      (extractedSrc.interdisciplinary &&
        (extractedSrc.interdisciplinary as any).workplaceApplications) ||
        (extractedSrc as any).workplaceApplications ||
        ""
    ),
  };

  const keyTakeaways = toList(
    extractedSrc.keyTakeaways || (extractedSrc as any).keyTakeaways || []
  );

  const references = {
    readings: toList(
      (extractedSrc.references && (extractedSrc.references as any).readings) ||
        (extractedSrc as any).readings ||
        []
    ),
    questionsForStudy: toList(
      (extractedSrc.references && (extractedSrc.references as any).questionsForStudy) ||
        (extractedSrc as any).questionsForStudy ||
        []
    ),
  };
...
function sanitizeSummaryText(s: string): string {
  ...
  if (text.startsWith("{")) {
    try {
      ...
      if (parsed && typeof parsed === "object" && (parsed as any).executiveSummary)
        text = String((parsed as any).executiveSummary).trim();
    } catch {}
  }
...
function parseLectureNotes(text: string) {
  ...
  lines.forEach((line) => {
    const isKnownHeader = headers.some(
      (h) => line.toUpperCase() === h || line.toUpperCase().includes(h + ":")
    );
